/**
 * db.js
 * لایه پایگاه داده با قابلیت خودترمیمی (Self-Healing Schema) و تصحیح خودکار خطاهای ستون دیتابیس
 */
...
const DB = {
  ...
  // ساخت و اصلاح ساختار ستون‌های دیتابیس
  async initCloudTables() {
    ...
    // افزودن خودکار ستون‌های جا مانده
    const columns = [
  ...
  async setupInitialAdmin(adminData) {
    ...
    if (this.isCloudConfigured()) {
      try {
        ...
        console.error('خطای ذخیره ادمین در ابری:', e);
      }
    }
  ...
  async insertEntry(entry) {
    ...
    if (this.isCloudConfigured()) {
      ...
      try {
        ...
        // در صورت عدم وجود ستون، ابتدا دیتابیس را ترمیم کرده و سپس دوباره تلاش می‌کند
  ...
  // ویرایش و ثبت خروج با قابلیت خودترمیمی هوشمند در برابر ارور no such column
  async updateRecord(id, updatedFields) {
    ...
    if (this.isCloudConfigured()) {
      ...
      try {
        ...
        if (e.message && e.message.includes('no such column')) {
          // ستون جا افتاده را به جدول تزریق کرده و دوباره کوئری را اجرا می‌کند
          await this.initCloudTables();
          await runUpdate();
        } else {
